exports('checkIfPlayerLookAtPed', (pedId, maxRange) => { return getFirstPedOnLookRay(maxRange) == pedId });
...
function getFirstPedOnLookRay(maxRange){
	...
	let t = [s[0] + lv[0] * maxRange, s[1] + lv[1] * maxRange, s[2] + lv[2] * maxRange] // cible
	...
	let ray = StartShapeTestRay( s[0], s[1], s[2], t[0], t[1], t[2],
			4,				// On cherche que des Ped : 1 map, 2 vehicules, 4 peds, 16 obj, ...
...
function getFirstMissionEntityOnLookRay(maxRange){
	...
	// On va calculer la position regardé par le joueur (la distance est calculée par rapport a la caméra)
function getLookVector(){
	
	let [Pitch, Roll, Yaw] = GetGameplayCamRot(0)

		// on passe les angles en radian et on arrange un peu les données pour aller dans la bonne direction
...
function vecMatMult4(a, B){
	...
	// DEBUG : on trace une ligne entre le joueur (pas la caméra) et le point ciblé lors de la recherche 
setTick(() => {
	
	if(_t[0] != 0){
		
			// DRAW_LINE
		Citizen.invokeNativeByHash( 0x00000000, 0xB3426BCC, _so[0], _so[1], _so[2], _t[0], _t[1], _t[2], 0, 0, 255, 255)
	}
})