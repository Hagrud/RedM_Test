  ...
  if( typeof(locationName) == "string"){
    ...
    if(locationName.length < 100){
      ...
      if( coords instanceof Array && coords.length == 3){
        ...
        emit("hagrud:ErrorFromClientInput", sourceId, 
		  `onNet("hagrud:save_place" ...)`,
		  `coords are not valids : '${coords}'`,
		  `Erreur dans le traitement des coordonnées`);
      }
    ...
    emit("hagrud:ErrorFromClientInput", sourceId, 
	  `onNet("hagrud:save_place" ...)`, 
	  `name is not a string`, `Erreur dans le traitement du nom`);
  }
...
onNet("hagrud:teleport_to_location", (sourceId, locationName) => {
  ...
  if( locationName.length < 100){
    ...
    emit("hagrud:ClientInvalidInput", sourceId, 
	  `onNet("hagrud:teleport_to_location" ...)`, 
	  `name is too long`, `Le nom est trop long`);
  }
...
function teleportTo( sourceId, pos ){
  if ( pos instanceof Array && pos.length == 3 ) {
    ...
    emit("hagrud:ErrorFromClientInput", sourceId, `teleportTo`, 
	  `coords are not valids : '${coords}'`,
	  `Erreur dans le traitement des coordonnées`);
  }
  return;
    ...
    exports["mysql-async"]["mysql_fetch_all"](
      sql_request, {}, function(res) {
        if(res.length > 0){
          ...
          emit("hagrud:ClientInvalidInput", sourceId, `teleportToLocation`, 
		    `location '${loc}' does not exist`, `La position est inconnue`);        
        }
      })
